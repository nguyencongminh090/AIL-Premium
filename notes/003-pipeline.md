...
- **Operation:**
  ...
  **1a. Tạo tập optic-rays** từ $\mathbf{t}$ đến mọi điểm $p$:
  ...
  $$\left\{\mathbf{L}_{\mathbf{t}\to p} \;\middle|\; \mathbf{o}_{\mathbf{t}\to p} = \mathbf{t},\; \mathbf{d}_{\mathbf{t}\to p} = \frac{\mathbf{x}_p - \mathbf{t}}{\|\mathbf{x}_p - \mathbf{t}\|};\; p = 1\ldots P\right\} \tag{10}$$
  ...
  **1b. Length Feature Field** $\mathcal{F}^p_\gamma$ (Fourier series trên $\mathbb{R}^1$):
  ...
  $$\mathcal{F}^p_\gamma(\gamma) = \mathbf{a}^{[\gamma]}_0 + \sum_{n=1}^{H_1}\!\left(\mathbf{a}^{[\gamma]}_{2n+1}\cos\!\left(\frac{2\pi\gamma}{\gamma_{max}}n\right) + \mathbf{a}^{[\gamma]}_{2n+2}\sin\!\left(\frac{2\pi\gamma}{\gamma_{max}}n\right)\right) \tag{9}$$

  **1c. Direction Feature Field** $\mathcal{F}^p_\mathbf{d}$ (Spherical Harmonics trên $\mathbb{S}^2$):

  $$\mathcal{F}^p_\mathbf{d}(\mathbf{d}) = \sum_{l=0}^{H_2}\sum_{m=-l}^{l} \mathbf{a}^{[\mathbf{d}]}_{lm}\, Y_{lm}(\mathbf{d}) \tag{7}$$

  trong đó $Y_{lm}(\cdot)$ là đa thức điều hòa cầu (harmonic polynomial) bậc $l$, hạng $m$ trên $\mathbb{S}^2$.

  **1d. Ray feature** kết hợp:

  $$\mathbf{f}_{\mathbf{t}\to p} = \mathcal{F}^p_\gamma(\gamma_{\mathbf{t}\to p}) + \mathcal{F}^p_\mathbf{d}(\mathbf{d}_{\mathbf{t}\to p}) \tag{11}$$
...
- **Operation:**
  ...
  **Visibility weight** của điểm $p$ đối với cell $c$:
  ...
  $$w_{c\leftarrow p} = \begin{cases} 1 & \text{if } p = \arg\min_{p^* \in \Omega_c} \gamma_{\mathbf{t}\to p^*} \\ 0 & \text{otherwise} \end{cases} \tag{13}$$

  trong đó $\Omega_c = \{p \mid c = \arg\max_{c^*}\, \mathbf{d}_{c^*}\cdot\mathbf{d}_{\mathbf{t}\to p}\}$ là tập điểm thuộc cell $c$.

  **Location descriptor**:

  $$\mathbf{Z}(\mathbf{t}) = \frac{1}{V_2}\sum_{c=1}^{V_2}\sum_{p=1}^{P} \frac{w_{c\leftarrow p}}{\sum_{p=1}^{P} w_{c\leftarrow p}}\, \mathbf{f}_{\mathbf{t}\to p} \tag{12}$$
...
- **Operation:**
  ...
  $$S_{loc}(\mathbf{t}) = \Phi_{loc}\!\left(\mathbf{Z}(\mathbf{t})\right) \in [0,1] \tag{14}$$
...
- **Operation:**
  ...
  **Hyper-ray** (biểu diễn 6DoF đơn nhất):
  ...
  $$\hat{\mathbf{L}} \in \mathbb{L}^6 = \left\{\hat{\mathbf{o}} \in \mathbb{R}^3,\; \mathbf{q} \in \mathbb{S}^3\right\} \tag{3}$$

  với $\mathbf{q} = (q_w, q_x, q_y, q_z)$, $\|\mathbf{q}\| = 1$ là quaternion đơn vị.

  **Ma trận quay** từ quaternion:

  $$\mathcal{R}(\mathbf{q}) = 2\cdot\begin{bmatrix} q_w^2+q_z^2-0.5 & q_xq_y - q_wq_z & q_wq_y+q_xq_z \\ q_wq_z+q_xq_y & q_w^2+q_y^2-0.5 & q_yq_z-q_wq_x \\ q_xq_z-q_wq_y & q_wq_z+q_yq_z & q_w^2+q_z^2-0.5 \end{bmatrix} \tag{4}$$

  **Auxiliary mapping** (collapse: hyper-ray → optic-ray):

  $$aux: \mathbb{L}^6 \to \mathbb{L}^5,\quad aux(\hat{\mathbf{L}}) = \left\{\mathbf{L} \mid \mathbf{o} = \hat{\mathbf{o}},\; \mathbf{d} = \mathcal{R}_3(\mathbf{q})\right\} \tag{15}$$

  trong đó $\mathcal{R}_3(\mathbf{q})$ là cột thứ ba của $\mathcal{R}(\mathbf{q})$.

  **Lift function** (optic-ray → tập hyper-rays liên tục):

  $$lift: \mathbb{L}^5 \to \mathbb{L}^6,\quad lift(\mathbf{L}) = \left\{\hat{\mathbf{L}} \mid aux(\hat{\mathbf{L}}) = \mathbf{L}_{\mathbf{t}\to p}\right\} \tag{16}$$
...
- **Operation:**
  ...
  **Direction Feature Field trên $\mathbb{S}^3$** (Harmonics Ray Encoder):
  ...
  $$\mathcal{F}^p_\mathbf{q}(\mathbf{q}) = \sum_{k=0}^{H_3}\sum_{l=0}^{k}\sum_{m=-l}^{l} \mathbf{a}^{[\mathbf{q}]}_{klm}\, Y_{klm}(\mathbf{q}) \tag{8}$$
  ...
  **Hyper-ray direction** đối với cell $c$ — tìm $\mathbf{q}^*$ nhỏ nhất thỏa:
  ...
  $$\mathbf{q}^c_{\mathbf{t}\to p} = \arg\min_{\mathbf{q}^*}\angle(\mathbf{q}^*)\quad \text{s.t.}\quad \mathcal{R}_3(\mathbf{q}^* \circ \mathbf{q}_c) = \mathbf{d}_{\mathbf{t}\to p} \tag{19}$$

  Nghiệm dạng đóng:

  $$\mathbf{q}^* = \left[\cos\!\left(\frac{|\mathbf{v}|}{2}\right),\; \sin\!\left(\frac{|\mathbf{v}|}{2}\right)\frac{\mathbf{v}}{|\mathbf{v}|}\right] \tag{20}$$

  **Cell feature** của Voronoi 3-sphere:

  $$\hat{\mathbf{Z}}_c(\mathbf{t}) = \sum_{p=1}^{P}\frac{w_{c\leftarrow p}}{\sum_{p=1}^{P}w_{c\leftarrow p}}\left(\mathcal{F}^p_\gamma(\gamma_{\mathbf{t}\to p}) + \mathcal{F}^p_\mathbf{q}(\mathbf{q}^c_{\mathbf{t}\to p})\right) \tag{18}$$
...
- **Operation:**
  ...
  **View Cropping** — weighted average với Gaussian kernel trên góc Euler:
  ...
  $$\hat{\mathbf{Z}}(\mathbf{R}, \mathbf{t}) = \frac{\sum_{c=1}^{V_3} w_c(\mathbf{R})\,\hat{\mathbf{Z}}_c(\mathbf{t})}{\sum_{c=1}^{V_3} w_c(\mathbf{R})} \tag{22}$$

  **Cell weight** — Gaussian kernel trên không gian góc Euler:

  $$w_c(\mathbf{R}) = \exp\!\left(-\mathbf{u}^T\,\mathrm{diag}(\sigma_\phi^2, \sigma_\theta^2, \sigma_\psi^2)^{-1}\,\mathbf{u}\right) \tag{23}$$
  ...
  **Sigma** tuyến tính theo FoV camera:
  ...
  $$[\sigma_\phi, \sigma_\theta, \sigma_\psi] = \lambda \cdot [\eta_h, \eta_v, \eta_r] \tag{24}$$
...
- **Operation:**
  ...
  $$S_{view}(\mathbf{R}, \mathbf{t}) = \Phi_{view}\!\left(\hat{\mathbf{Z}}(\mathbf{R}, \mathbf{t})\right) \in [0, 1] \tag{25}$$
...
- **Operation:**
  ...
  $$S_{final}(\mathbf{R}, \mathbf{t}) = S_{loc}(\mathbf{t}) \times S_{view}(\mathbf{R}, \mathbf{t}) \tag{26}$$
...
$$\mathcal{L}_{loc} = -\sum_i \log\!\left(S_{loc}(\mathbf{t}_i^+)\right) - \sum_j \log\!\left(1 - S_{loc}(\mathbf{t}_j^-)\right) \tag{27}$$
...
$$\mathcal{L}_{view} = -\sum_i \log\!\left(S_{view}(\mathbf{R}_i^+, \mathbf{t}_i^+)\right) - \sum_j \log\!\left(1 - S_{view}(\mathbf{R}^-, \mathbf{t}_i^+)\right) \tag{28}$$
